...
from tensorflow_addons.image import resampler as contrib_resampler

from . import cap_prob
import geo_block
...
class CapsuleMaterialEncoder(snt.AbstractModule):
  ...
  def __init__(self,
    ...
    super(CapsuleMaterialEncoder, self).__init__()
  ...
  def _build(self, x):
    ...
    x = tf.transpose(tf.reshape(x, [1, batch_size, 16, 16]), [1, 2, 3, 0])
    ...
    if self._encoder_type == 'linear':
      ...
      h = snt.AddBias(bias_dims=[2, 3])(img_embedding)
      #biases = tf.get_variable('biases', shape=[1, 2, 3], initializer=tf.zeros_initializer())
      #h = tf.nn.bias_add(img_embedding, biases)
    ...
    if self._noise_scale > 0.:
      pres_logit += ((tf.random.uniform(tf.shape(pres_logit)) - .5)
                     * self._noise_scale)


    pres = tf.nn.softmax(pres_logit, axis = -1)
    pose = geo_block.geometric_transform(pose, self._similarity_transform)
    return self.OutputTuple(pose, feature, pres, pres_logit, img_embedding, h)


def choose_nonlinearity(name):
  nonlin = getattr(geo_block, name, getattr(tf.nn, name, None))
...
class TemplateBasedMaterialDecoder(snt.AbstractModule):
  ...
  def __init__(self,
               ...
               template_nonlin='selu',
               color_nonlin='selu',
               use_alpha_channel=False):

    super(TemplateBasedMaterialDecoder, self).__init__()
  ...
  def make_templates(self, n_templates=None, template_feature=None):
    ...
    if template_feature is not None:
      ...
      if self._colorize_templates:
        mlp = snt.BatchApply(snt.nets.MLP([36, self._n_channels]))
        template_color = mlp(template_feature)[:, :, tf.newaxis, tf.newaxis]

        if self._color_nonlin == geo_block.relu1:
          template_color += .99
  ...
  def _build(self,
    ...
    #batch_s = 36  # Temporary parameters
    num_batch = tf.shape(pose)[0]
    if templates.shape[0] == 1:

      templates = snt.TileByDim([0], [num_batch])(templates)
    ...
    resampler = snt.BatchApply(contrib_resampler)
    ...
    if presence is not None:
      presence = tf.concat([presence, tf.ones([num_batch, 1])], axis=1)

    if True:  # pylint: disable=using-constant-test

      if self._use_alpha_channel:
        template_mixing_logits = snt.TileByDim([0], [num_batch])(
    ...
    if self._output_pdf_type == 'mixture':
      template_mixing_logits += make_brodcastable(
          geo_block.safe_log(presence), template_mixing_logits)

      rec_pdf = cap_prob.MixtureDistribution(template_mixing_logits,
