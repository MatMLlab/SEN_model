...
class Forward_predictor(object):
    ...
    def forward_predictor(self, nbond = None, nstate = None, atom_emb = 108, emb_dim = 80):

        """build prediction model with capsule graphs constructed using the tensorflow, sonnet, and keras backend"""
        ...
        state_vec = Input(shape=(None, nstate), name="state_feature_input")
        ...
        index_inp = atom_index, nei_index, atom_sou
    ...
    def train_model(self):
        ...
        if self.mode not in [['hse'], ['scan'], ['gllb']]:
            raise ValueError('`DFT Mode` must in `pbe`, `hse`, `scan` or `gllb`')
        ...
        self.forward_predictor(nbond= 100, nstate = None)
